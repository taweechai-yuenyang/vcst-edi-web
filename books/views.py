...
from .models import Book, EDIReviseType, RefType, ReviseBook
from .serializers import BookSerializer, ReviseBookSerializer, ReviseTypeSerializer, RefTypeSerializer
...
class ReviseBookListApiView(APIView):
    # add permission to check if user is authenticated
    permission_classes = [permissions.IsAuthenticated]
    ...
    def get(self, request, *args, **kwargs):
        ...
        id = self.request.query_params.get('id')
        if id:
            obj = ReviseBook.objects.get(id=id)
            serializer = ReviseBookSerializer(obj)
            return Response(serializer.data, status=status.HTTP_200_OK)
        
        obj = ReviseBook.objects.all()
        serializer = ReviseBookSerializer(obj, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    ...
    def post(self, request, *args, **kwargs):
        ...
        Create the Todo with given todo data
        '''
        rType = RefType.objects.get(code=request.data.get('ref_type_id'))
        bType = Book.objects.get(name=request.data.get('book_id'))
        obj = request.POST.copy()
        obj['ref_type_id'] = rType.id
        obj['book_id'] = bType.id
        
        
        serializer = ReviseBookSerializer(data=obj)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
