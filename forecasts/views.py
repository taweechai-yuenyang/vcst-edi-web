...
def download_open_pds(request, id):
    dte = datetime.now()
    file_name = f"export_forecast_{dte.strftime('%Y%m%d%H%M')}"
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = f'attachment; filename="{file_name}.xls"'
    ...
    # Fetch Data
    head = PDSHeader.objects.get(id=id)
    # Merge cells for the header
    ws.write_merge(0, 0, 0, 4, head.supplier_id.name, style=header_style)  # Merging cells (0,0) to (0,2)
    ws.col(0).height = 5000
    ...
    # # Sheet body, remaining rows
    # rows =  ForecastErrorLogs.objects.filter(file_name=id).values_list("part_code","part_no","part_name","supplier","model","rev_0","rev_1","rev_2","rev_3","remark",)
    rows = PDSDetail.objects.filter(pds_header_id=id)
    i = 0
    row_num = 2
    for r in rows:
        ws.write(row_num, 0, str(i + 1), table_style)
        ws.write(row_num, 1, r.forecast_detail_id.import_model_by_user, table_style)
        ws.write(row_num, 2, r.forecast_detail_id.product_id.code, table_style)
        ws.write(row_num, 3, r.forecast_detail_id.product_id.name, table_style)
        ws.write(row_num, 4, r.forecast_detail_id.request_qty, table_style)
        i += 1
        row_num += 1
